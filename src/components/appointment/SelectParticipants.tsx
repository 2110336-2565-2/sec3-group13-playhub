...
export default function SelectParticipants(props: props) {
  ...
  const addParticipantBorderStyle: string = props.participantCountError ? COLOR_CODE.ERROR : COLOR_CODE.BLACK
      ...
      <Grid container spacing={1} style={{ marginLeft: -5 }}>
        ...
        {props.availableParticipants && props.availableParticipants.length != 0 && (
          <Grid item key={props.availableParticipants.length + 1}>
            <IconButton onClick={handleOpenMenu} style={{ border: "2px dashed", borderColor: addParticipantBorderStyle }}>
              <AddIcon color={props.participantCountError ? "error" : "secondary"} fontSize="large" />
            </IconButton>
          </Grid>
        )}
      </Grid>
